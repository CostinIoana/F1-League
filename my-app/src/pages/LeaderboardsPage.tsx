...
export function LeaderboardsPage() {
  ...
  const userStandings = useMemo(() => {
    if (!selectedSeason) {
      return [] as Array<{ email: string; points: number; locked: boolean }>;
    }
    return getUserSelectionsFromStorage(selectedSeason.id)
      .map((userSelection) => ({
        email: userSelection.email,
        points: userSelection.selectedSlotIds.reduce(
          (sum, slotId) => sum + (pointsBySlot[slotId] ?? 0),
          0
        ),
      ...
      <Surface tone="subtle">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <Button
            type="button"
            size="sm"
            variant={scoreScope === SEASON_TOTAL_SCOPE ? "solid" : "outline"}
          ...
          {selectedSeason.races.map((race) => (
            <Button
              ...
              type="button"
              size="sm"
              variant={scoreScope === race.id ? "solid" : "outline"}
              ...
              userStandings.map((user, index) => (
                ...
                >
                  <div className="flex items-center gap-2 text-sm text-[var(--color-neutral-800)]">
                    ...
                    {user.locked && <Badge tone="neutral">Locked</Badge>}
                  </div>
